...
### 1. DNS Configuration (Host Machine – Testing Only)

For quick testing from a single workstation, configure wildcard DNS resolution
to point to the server running this stack.

This can be done via:
- `/etc/hosts` (limited, manual)
- Local DNS resolver
...
### 2. DNS Configuration (Firewall – Production / LAN)

For shared or production-like environments, DNS should be handled centrally
...
- Firewall IP: `10.2.0.1`
- Services host IP: `10.2.0.10`
...
Explanation:
- `pfsense.inethilocal.net` resolves directly to the firewall
- All other `*.inethilocal.net` names resolve to the services host
